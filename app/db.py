...
CREATE INDEX IF NOT EXISTS idx_workspace_invites_topic ON workspace_invites(topic_id);
...
CREATE TABLE IF NOT EXISTS agent_instances (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    agent_type_id BIGINT NOT NULL REFERENCES agent_types(id),
...
CREATE TABLE IF NOT EXISTS topics (
    ...
    agent_intervention_mode TEXT NOT NULL DEFAULT 'auto'
        CHECK (agent_intervention_mode IN ('auto', 'mentions', 'silent')),
    shared_context_mode TEXT NOT NULL DEFAULT 'topic_with_files'
        CHECK (shared_context_mode IN ('topic_only', 'topic_with_files')),
...
CREATE INDEX IF NOT EXISTS idx_topics_visible ON topics(workspace_id, archived_at, deleted_at);
...
CREATE TABLE IF NOT EXISTS attachments (
    ...
    uploaded_by_human_id BIGINT REFERENCES humans(id),
...
CREATE INDEX IF NOT EXISTS idx_attachments_topic ON attachments(topic_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_attachments_workspace ON attachments(workspace_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_attachment_chunks_topic ON attachment_chunks(topic_id, attachment_id, chunk_index);
...
CREATE TABLE IF NOT EXISTS topic_share_links (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_topic_share_links_topic
    ON topic_share_links(topic_id, created_at DESC);
"""
...
def _migrate_agent_roles_to_types(conn) -> None:
    """Rename legacy agent identity schema before the main schema runs.

    Public API fields still use ``role`` for compatibility with existing
    gateway tokens and CLI flags, but the DB name should describe what the rows
    really are: supported agent adapter types such as claude or codex.
    """
    has_old_table = conn.execute(
        """
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'agent_roles'
        """
    ).fetchone()
    has_new_table = conn.execute(
        """
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'agent_types'
        """
    ).fetchone()

    if has_old_table and not has_new_table:
        conn.execute("ALTER TABLE agent_roles RENAME TO agent_types")
    ...
    if old_col and not new_col:
        ...
        conn.execute("ALTER TABLE agent_instances RENAME COLUMN role_id TO agent_type_id")

    old_human_col = conn.execute(
        """
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public'
          AND table_name = 'agent_instances'
          AND column_name = 'human_id'
        """
    ).fetchone()
    new_owner_col = conn.execute(
        """
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public'
          AND table_name = 'agent_instances'
          AND column_name = 'owner_human_id'
        """
    ).fetchone()

    if old_human_col and not new_owner_col:
        conn.execute("DROP INDEX IF EXISTS idx_agent_instances_human")
        conn.execute("ALTER TABLE agent_instances RENAME COLUMN human_id TO owner_human_id")
    ...
    if ai_cols is None:
        ...
        conn.execute(
            ...
            SET workspace_id = (
                ...
                WHERE wm.human_id = ai.owner_human_id
...
def _migrate_humans_guest_flag(conn) -> None:
    """Add humans.is_guest on databases created before guest invites."""
    conn.execute(
...
def _migrate_topics_lifecycle(conn) -> None:
    ...
    conn.execute(
        "ALTER TABLE IF EXISTS topics ADD COLUMN IF NOT EXISTS agent_intervention_mode TEXT "
        "NOT NULL DEFAULT 'auto' CHECK (agent_intervention_mode IN ('auto', 'mentions', 'silent'))"
    )
    conn.execute(
        "ALTER TABLE IF EXISTS topics ADD COLUMN IF NOT EXISTS shared_context_mode TEXT "
        "NOT NULL DEFAULT 'topic_with_files' CHECK (shared_context_mode IN ('topic_only', 'topic_with_files'))"
    )
    ...
    conn.execute(
        ...
        "ON topics(workspace_id, archived_at, deleted_at)"
    )
...
def _migrate_workspace_invites_topic(conn) -> None:
    """Allow invite links to carry an optional topic join target."""
    table = conn.execute(
        "SELECT 1 FROM information_schema.tables "
    ...
    if table is None:
        return
    conn.execute("ALTER TABLE IF EXISTS workspace_invites ADD COLUMN IF NOT EXISTS topic_id BIGINT")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_workspace_invites_topic "
        "ON workspace_invites(topic_id)"
    )
...
def _cleanup_deleted_agent_memberships(conn) -> None:
    """Remove current membership rows for agents that have already been retired."""
    conn.execute(
        """
        DELETE FROM topic_participants tp
        USING agent_instances ai
        WHERE tp.participant_type = 'agent'
    ...
    conn.execute(
        """
        DELETE FROM workspace_agent_members wam
        USING agent_instances ai
        WHERE wam.agent_instance_id = ai.id
          AND ai.deleted_at IS NOT NULL
        """
    )
...
def _migrate_agent_instances_independent(conn) -> None:
    ...
    Production can be wiped for this change, but tests and local developer DBs
    may still have the old shape. Preserve old rows by copying workspace_id into
    workspace_agent_members before dropping the column.
    """
    conn.execute("ALTER TABLE IF EXISTS agent_instances ADD COLUMN IF NOT EXISTS display_name TEXT")
    conn.execute("ALTER TABLE IF EXISTS agent_instances ADD COLUMN IF NOT EXISTS paused_at TIMESTAMPTZ")
    conn.execute("ALTER TABLE IF EXISTS agent_instances ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ")

    cols = conn.execute(
        """
        SELECT column_name FROM information_schema.columns
        WHERE table_name = 'agent_instances'
          AND column_name IN ('human_id', 'owner_human_id', 'workspace_id')
        """
    ).fetchall()
    col_names = {r["column_name"] for r in cols}

    if "owner_human_id" not in col_names and "human_id" in col_names:
        conn.execute("ALTER TABLE agent_instances RENAME COLUMN human_id TO owner_human_id")
        col_names.discard("human_id")
        col_names.add("owner_human_id")
    ...
    if "workspace_id" in col_names:
        conn.execute(
            ...
            SELECT workspace_id, id, owner_human_id
        ...
        conn.execute("DROP INDEX IF EXISTS idx_agent_instances_workspace")
        conn.execute(
            """
            DO $$
            DECLARE fk_name TEXT;
            BEGIN
              ...
              WHERE conrelid = 'agent_instances'::regclass
                AND contype = 'f'
                AND pg_get_constraintdef(oid) LIKE '%workspace_id%';
              IF fk_name IS NOT NULL THEN
                EXECUTE 'ALTER TABLE agent_instances DROP CONSTRAINT ' || quote_ident(fk_name);
              END IF;
            END $$;
            """
        )
        conn.execute("ALTER TABLE agent_instances DROP COLUMN workspace_id")
...
def init_db() -> None:
    ...
    with _init_lock:
        ...
        with connect() as conn:
            ...
            _migrate_workspace_invites_topic(conn)
            ...
            _cleanup_deleted_agent_memberships(conn)
